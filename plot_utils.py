...
def create_algorithm_legend(algorithms: list,
                           include_style_legend: bool = False,
                           empirical_label: str = 'Empirical',
                           theory_label: str = 'Theory') -> Tuple[list, list]:
    """
    Create legend elements for algorithms with optional style legend.
    
    Args:
        algorithms: List of algorithm names
        include_style_legend: Whether to include empirical/theory style legend
        empirical_label: Label for empirical (solid) lines
        theory_label: Label for theory (dashed) lines
        
    Returns:
        Tuple of (algorithm_legend_elements, style_legend_elements)
    """
    # Algorithm legend
    alg_elements = []
    ...
    # Style legend
    style_elements = []
    if include_style_legend:
        style_elements = [
            Line2D([0], [0], color='black', lw=2, linestyle='-',
                   label=empirical_label, markersize=5),
            Line2D([0], [0], color='black', lw=2, linestyle='--',
                   label=theory_label, markersize=5),
        ]
    
    return alg_elements, style_elements
...
def plot_comparison(
    ...
    eigenvalue_style: str = "min_med_max"
):
    """
    Plot comparison of multiple algorithms showing fitness and Hessian trace over time.
    
    Args:
        results: Dict mapping algorithm labels to tuples of 
                 (time, position_history, fitness, hessian_trace)
        title: Plot title
        sigma: Sigma value for gradient ascent display
        plot_fitness: Whether to show fitness vs time plot
        plot_zoomed: Whether to show zoomed-in inset on fitness plot
        plot_contours: Whether to add trajectory contour map
        fitness_function: Function for contour plot (required if plot_contours=True)
        plot_theory: Whether to add theoretical predictions
        learning_rate: Learning rate for theoretical calculation
        initial_x: Initial x position for theoretical calculation
        plot_curvature: Whether to plot curvature (True) or x position (False)
        plot_eigenvalues: Whether to plot eigenvalue magnitude stats vs time
        hessian_func: Hessian function required when plot_eigenvalues=True
        color_map: Optional mapping of label -> color override
        eigenvalue_style: "min_med_max" or "median_band"
    """
    try:
        import jax.numpy as jnp
        import jax
    ...
    # Calculate number of plots based on what's requested
    num_plots = int(plot_fitness) + 1 + int(plot_eigenvalues) + int(plot_contours)
    
    fig, axes = create_figure(n_cols=num_plots)
    
    axes_list = list(axes) if isinstance(axes, (list, np.ndarray)) else [axes]
    ax_fitness = None
    ax_curvature = None
    ax_eigenvalues = None
    ax_contours = None
    idx = 0
    if plot_fitness:
        ax_fitness = axes_list[idx]
        idx += 1
    ax_curvature = axes_list[idx]
    idx += 1
    if plot_eigenvalues:
        ax_eigenvalues = axes_list[idx]
        idx += 1
    if plot_contours:
        ax_contours = axes_list[idx]
    
    if plot_contours and fitness_function is None:
        raise ValueError("fitness_function must be provided when plot_contours is True.")
    if plot_eigenvalues and hessian_func is None:
        raise ValueError("hessian_func must be provided when plot_eigenvalues is True.")

    # No figure title for this plot

    legend_elements = []
    all_fitness_data = []
    
    for i, (label, stats) in enumerate(results.items()):
        ...
        if plot_fitness and ax_fitness is not None:
            ax_fitness.plot(time_data, fitness_data, color=color, linestyle=linestyle, alpha=0.9)
        
        y = jnp.abs(hessian_trace_data) if plot_curvature else pos_history[:, 0]
        ax_curvature.plot(time_data, y, color=color, linestyle=linestyle, alpha=0.9)

        if plot_eigenvalues and ax_eigenvalues is not None:
            try:
            ...
            eig_min = np.min(eigvals_abs, axis=1)
            eig_med = np.median(eigvals_abs, axis=1)
            eig_max = np.max(eigvals_abs, axis=1)

            if eigenvalue_style == "median_band":
                eig_low = np.percentile(eigvals_abs, 10, axis=1)
                eig_high = np.percentile(eigvals_abs, 90, axis=1)
                ax_eigenvalues.fill_between(
                    time_data, eig_low, eig_high, color=color, alpha=0.15, linewidth=0
                )
                ax_eigenvalues.plot(time_data, eig_med, color=color, linestyle='-', alpha=0.9)
            else:
                ax_eigenvalues.plot(time_data, eig_min, color=color, linestyle='--', alpha=0.7)
                ax_eigenvalues.plot(time_data, eig_med, color=color, linestyle='-', alpha=0.9)
                ax_eigenvalues.plot(time_data, eig_max, color=color, linestyle=':', alpha=0.7)

        if plot_contours and ax_contours is not None:
            ax_contours.plot(pos_history[:, 0], pos_history[:, 1], color=color, linestyle=linestyle, alpha=0.9)

        if len(results) > 1:
            legend_elements.append(Line2D([0], [0], color=color, lw=2, linestyle=linestyle, 
    ...
    ax_curvature.set_ylabel("Curvature" if plot_curvature else "x")
    set_equal_aspect(ax_curvature, ratio_based=True)
    style_axis(ax_curvature)

    if ax_eigenvalues is not None:
        ax_eigenvalues.set_xlabel("Generation")
        ax_eigenvalues.set_ylabel(r"Eigenvalue magnitude $|\lambda|$")
        set_equal_aspect(ax_eigenvalues, ratio_based=True)
        style_axis(ax_eigenvalues)
        if eigenvalue_style == "median_band":
            ax_eigenvalues.legend(
                handles=[
                    Line2D([0], [0], color='black', linestyle='-', label='median'),
                    mpatches.Patch(color='black', alpha=0.15, label='10–90%')
                ],
                frameon=False,
                fontsize=8,
                loc='upper right'
    ...
    if plot_contours and ax_contours is not None:
        add_fitness_contours(ax_contours, fitness_function, xlim=(-3, 3), ylim=(-3, 3))
        ax_contours.set_xlabel("x")
        ax_contours.set_ylabel("y")
        set_equal_aspect(ax_contours)
        style_axis(ax_contours)
...
# =============================================================================
# THEORY ELLIPSE PLOTS
# =============================================================================

def plot_theory_ellipses_on_landscape(
    theory_statistics: Tuple,
    fitness_function,
    snapshot_times: Optional[List[int]] = None,
    params: Optional[Dict] = None,
    ...
    """
    Plot theoretical distribution ellipses on fitness landscape contours.
    
    Args:
        theory_statistics: Tuple from run_manifold_dynamics_theoretical (time, mu, a, b)
        fitness_function: Callable that evaluates fitness at a point
        snapshot_times: List of time indices to show ellipses
        params: Dictionary with simulation parameters
        save_fig: Whether to save the figure
    ...
    """
    # Unpack theory statistics
    time_points, mu_history, a_history, b_history = theory_statistics
    ...
    if snapshot_times is None:
        num_snapshots = 5
        max_t = len(time_points) - 1
        snapshot_times = [int(i * max_t / (num_snapshots - 1)) for i in range(num_snapshots)]
    ...
    fig, ax = create_figure(n_cols=1, n_rows=1, width_per_panel=6, height_per_panel=6)
    fig.patch.set_facecolor('white')
    
    add_fitness_contours(ax, fitness_function, xlim=(-4, 4), ylim=(-3, 3),
                        resolution=200, levels=21, vmin=0, vmax=10)
    ...
    for i, t in enumerate(snapshot_times):
        ...
        center = (mu_t, 0.0)
        cov_matrix = np.array([[a_t, 0], [0, b_t]])
        
        add_covariance_ellipse(ax, center, cov_matrix, n_sigma=n_sigma,
                              color=colors[i], linewidth=1.5, alpha=0.8, fill=False, zorder=10 + i)
        
        scatter_point(ax, mu_t, 0.0, color=colors[i], size=40, marker='o', zorder=11 + i)
    
    # Trajectory of means
    ax.plot(mu_history, np.zeros_like(mu_history), 
            color='gray', linewidth=1, linestyle='--', alpha=0.5, zorder=5)
    
    style_axis(ax, remove_top_right=True)
    ...
    # Colorbar
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=0, vmax=time_points[-1]))
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax, shrink=0.8, label='Generation $t$')
    
    # Legend
    legend_elements = [
        Line2D([0], [0], color=colors[i], linewidth=2, label=f't={snapshot_times[i]}')
        for i in range(len(snapshot_times))
    ]
    ...
    if save_fig:
        beta = params.get('beta', 1.0)
        mutation_std = params.get('mutation_std', 0.1)
        num_iter = params.get('num_iterations', len(time_points) - 1)
        filename = f"theory_ellipses_landscape_iter{num_iter}_mut{mutation_std:.3f}_beta{beta:.2f}.jpeg"
        save_figure(fig, filename, format='jpeg')
...
def plot_mutation_std_comparison(
    comparison_results: Dict,
    fitness_function,
    save_fig: bool = True
):
    """
    Plot results from run_mutation_std_comparison.
    
    Args:
        comparison_results: Dictionary returned by run_mutation_std_comparison
        fitness_function: Fitness function for contour plots
        save_fig: Whether to save the figure
    """
    results = comparison_results['results']
    initial_population = comparison_results['initial_population']
    initial_mean = comparison_results['initial_mean']
    params = comparison_results['params']
    
    mutation_std_values = params['mutation_std_values']
    
    fig, axes = create_figure(n_cols=3)
    ax_distributions, ax_trajectories, ax_curvature = axes

    colors = get_colors_for_values(mutation_std_values)
    legend_elements = []

    # Add fitness contours
    add_fitness_contours(ax_distributions, fitness_function, xlim=(-3, 3), ylim=(-3, 3))
    add_fitness_contours(ax_trajectories, fitness_function, xlim=(0, 2), ylim=(-1, 1))

    middle_idx = 0#len(mutation_std_values) // 2
    ...
    for i, mutation_std in enumerate(mutation_std_values):
        ...
        time_data, pos_history, fitness_data, hessian_trace_data, covariance_matrices = evo_stats
        
        # Calculate curvature
        import jax.numpy as jnp
        curvature_data = jnp.linalg.norm(pos_history, axis=1)
        
        plot_timeseries(ax_curvature, np.array(time_data), np.array(curvature_data), color=color)
        plot_trajectory(ax_trajectories, np.array(pos_history), color=color)
        scatter_point(ax_trajectories, float(pos_history[-1, 0]), float(pos_history[-1, 1]), 
                     color=color, size=20, zorder=3)
    ...
    # Plot distributions
    if initial_population is not None and final_population is not None:
        initial_color = COLORS[0]
        final_color = COLORS[3]
        
        scatter_population(ax_distributions, np.array(initial_population), color=initial_color)
        scatter_population(ax_distributions, np.array(final_population), color=final_color)
        
        import jax.numpy as jnp
        scatter_point(ax_distributions, 
                     float(jnp.mean(initial_population[:, 0])), 
                     float(jnp.mean(initial_population[:, 1])),
                     color=initial_color, size=50)
        scatter_point(ax_distributions, 
                     float(jnp.mean(final_population[:, 0])), 
                     float(jnp.mean(final_population[:, 1])),
                     color=final_color, size=50)

    ax_distributions.set_xlim(-3, 3)
    ax_distributions.set_ylim(-3, 3)
    ax_trajectories.set_xlim(0, 2)
    ax_trajectories.set_ylim(-1, 1)

    for ax in axes:
        style_axis(ax)

    ax_distributions.set_xlabel("x")
    ax_distributions.set_ylabel("y")
    ax_trajectories.set_xlabel("x")
    ax_trajectories.set_ylabel("y")
    ax_curvature.set_xlabel("Generation")
    ax_curvature.set_ylabel("Curvature")

    set_equal_aspect(ax_distributions)
    set_equal_aspect(ax_trajectories)
    set_equal_aspect(ax_curvature, ratio_based=True)
    ...
    if len(mutation_std_values) > 1:
        ncols = min(len(mutation_std_values), 4)
        add_legend(fig, legend_elements, ncol=ncols)
...
def plot_ed_gd_sgd_trajectories(
    comparison_results: Dict,
    fitness_function,
    F_MAX: float = 10.0,
    save_fig: bool = True
    ...
    """
    Plot fitness, curvature, and trajectory for ED, GLD, SGD.
    
    Creates 2 rows x 3 columns:
    - Top row: Single example trajectory
    - Bottom row: Average trajectory
    - Columns: Trajectory, Curvature, Fitness
    
    Args:
        comparison_results: Dictionary from run_ed_gd_sgd_comparison
        fitness_function: Fitness function for contour plots
        F_MAX: Maximum fitness for contours
        save_fig: Whether to save figure
    """
    import jax
    import jax.numpy as jnp
    
    example_stats = comparison_results['example_statistics']
    average_stats = comparison_results['average_statistics']
    params = comparison_results['params']
    
    # Styling parameters (matching plot_ed_vs_full_nes_summary)
    trajectory_linewidth = 1.5
    
    fig = plt.figure(figsize=(12, 8))
    
    # Create GridSpec layout (matching plot_ed_vs_full_nes_summary structure)
    outer_gs = gridspec.GridSpec(2, 1, figure=fig, height_ratios=[1, 1], 
                                 hspace=0.4, top=0.85, bottom=0.12, left=0.12, right=0.88)
    
    top_gs = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=outer_gs[0], wspace=0.4)
    bottom_gs = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=outer_gs[1], wspace=0.4)
    ...
    for row_idx, (stats_dict, gs) in enumerate([(example_stats, top_gs), (average_stats, bottom_gs)]):
        ax_trajectory = fig.add_subplot(gs[0])
        ax_curvature = fig.add_subplot(gs[1])
        ax_fitness = fig.add_subplot(gs[2])
        
        all_axes.extend([ax_trajectory, ax_curvature, ax_fitness])
        
        # Add contours to trajectory plot
        ax_trajectory.contour(X, Y, Z, levels=np.linspace(0, F_MAX, 21),
                             colors='black', linewidths=0.5, alpha=0.3)
        
        for alg_name in ['ED', 'GLD', 'SGD']:
            stats = stats_dict[alg_name]
            color = get_algorithm_color(alg_name)
            
            # Unpack stats (first 4 elements)
            time_data, pos_history, fitness_data, hessian_trace_data = stats[:4]
            time_data = np.array(time_data)
            pos_history = np.array(pos_history)
            fitness_data = np.array(fitness_data)
            curvature_data = np.abs(np.array(hessian_trace_data))
            
            # Plot trajectory with line
            ax_trajectory.plot(pos_history[:, 0], pos_history[:, 1], 
                              color=color, linewidth=trajectory_linewidth)
            
            # Plot curvature
            ax_curvature.plot(time_data, curvature_data, color=color, linewidth=trajectory_linewidth)
            
            # Plot fitness
            ax_fitness.plot(time_data, fitness_data, color=color, linewidth=trajectory_linewidth)
                  
            # Only add legend elements once
            if row_idx == 0:
                legend_elements.append(Line2D([0], [0], color=color, lw=2, label=alg_name))
        ...
        if row_idx == 0:
            ax_trajectory.set_ylabel('$y$')
        ax_trajectory.set_xlim(-3, 3)
        ax_trajectory.set_ylim(-3, 3)
        style_axis(ax_trajectory)
        ax_trajectory.set_aspect('equal', adjustable='box')
        
        # Style curvature axis
        if row_idx == 1:
            ax_curvature.set_xlabel('Generation')
        ax_curvature.set_ylabel('Curvature')
        style_axis(ax_curvature)
        ax_curvature.set_aspect(1.0/ax_curvature.get_data_ratio(), adjustable='box')
        ...
        ax_fitness.set_ylabel('Fitness')
        style_axis(ax_fitness)
        ax_fitness.set_xlim(0,20)
        ax_fitness.set_aspect(1.0/ax_fitness.get_data_ratio(), adjustable='box')
    
    # Add subplot labels (matching plot_ed_vs_full_nes_summary placement)
    labels = ['(a)', '(b)', '(c)', '(d)', '(e)', '(f)']
    for idx, ax in enumerate(all_axes):
        ax.text(-0.1, 1.25, labels[idx], transform=ax.transAxes, 
               fontsize=14, va='top', ha='right')
    
    # Add legend
    fig.legend(handles=legend_elements, loc='upper center', ncol=3, 
              fontsize=11, bbox_to_anchor=(0.5, 1.02), frameon=False)
    
    fig.patch.set_facecolor('white')
    plt.show()
    
    if save_fig:
        filename = f"ed_gd_sgd_trajectories_iter{params['num_iterations']}_pop{params['population_size']}_mut{params['mutation_std']:.3f}_beta{params['beta']:.2f}.jpeg"
        save_figure(fig, filename)
    
    return fig


def plot_ed_gd_sgd_avg_dynamics_and_populations(
    avg_dynamics_results: Dict,
    ...
    """
    Plot average dynamics (trajectory, curvature, fitness) and final populations.
    
    Creates 2 rows x 3 columns:
    - Top row: Average dynamics (trajectory, curvature, fitness)
    - Bottom row: Final populations (ED, GLD, SGD)
    
    Args:
        avg_dynamics_results: Results dict for average dynamics (top row)
        population_results: Results dict for final populations (bottom row)
        fitness_function: Fitness function for contour plots
        F_MAX: Maximum fitness for contours
        save_fig: Whether to save figure
    """
    ...
    average_stats = avg_dynamics_results['average_statistics']
    populations = population_results['populations']
    avg_params = avg_dynamics_results['params']
    pop_params = population_results['params']
    ...
    outer_gs = gridspec.GridSpec(
        ...
        hspace=0.65, top=0.85, bottom=0.12, left=0.12, right=0.88
    )
    top_gs = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=outer_gs[0], wspace=0.4)
    bottom_gs = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=outer_gs[1], wspace=0.4)
    ...
    # --- Top row: average dynamics ---
    ax_trajectory = fig.add_subplot(top_gs[0])
    ax_curvature = fig.add_subplot(top_gs[1])
    ax_fitness = fig.add_subplot(top_gs[2])
    all_axes.extend([ax_trajectory, ax_curvature, ax_fitness])
    ...
    for i, (name, pop, color) in enumerate(methods):
        ...
        ax.contour(X, Y, Z, levels=np.linspace(0, F_MAX, 21),
                   colors='black', linewidths=0.5, alpha=0.3)
        scatter_population(ax, np.array(pop), color=color)
    ...
    if save_fig:
        filename = (
            "ed_gd_sgd_avg_dynamics_and_populations_"
            f"avg_iter{avg_params['num_iterations']}_pop{avg_params['population_size']}_"
            f"avg_mut{avg_params['mutation_std']:.3f}_avg_beta{avg_params['beta']:.2f}_"
            f"pop_iter{pop_params['num_iterations']}_pop{pop_params['population_size']}_"
            f"pop_mut{pop_params['mutation_std']:.3f}_pop_beta{pop_params['beta']:.2f}.jpeg"
        )
        save_figure(fig, filename)
...
def plot_covariance_alignment_over_time(
    alignment_data: Dict,
    save_fig: bool = False,
    filename_prefix: str = "covariance_alignment"
) -> plt.Figure:
    """
    Plot covariance alignment with Hessian eigenvectors over time.
    
    Shows:
    - (a) Mean projected variance in flat vs sharp directions over time
    - (b) Mean absolute Hessian eigenvalue for flat vs sharp groups over time
    - (c) Ratio of flat to sharp variance over time
    
    Args:
        alignment_data: Output from compute_covariance_hessian_alignment()
        save_fig: Whether to save the figure
    ...
    """
    time = alignment_data['time']
    flat_var = alignment_data['flat_variance']
    sharp_var = alignment_data['sharp_variance']
    hess_eigs = alignment_data['hessian_eigenvalues']
    flat_k = alignment_data['flat_k']
    sharp_k = alignment_data['sharp_k']
    
    # Compute mean absolute eigenvalue for flat vs sharp
    flat_curv = np.mean(np.abs(hess_eigs[:, :flat_k]), axis=1)
    sharp_curv = np.mean(np.abs(hess_eigs[:, -sharp_k:]), axis=1)
    
    fig, axes = create_figure(n_cols=3)
    ax_var, ax_curv, ax_ratio = axes
    
    # Colors
    flat_color = COLORS[0]   # Blue
    sharp_color = COLORS[3]  # Red
    
    # Panel (a): Variance over time
    ax_var.plot(time, flat_var, color=flat_color, linewidth=2)
    ax_var.plot(time, sharp_var, color=sharp_color, linewidth=2)
    ax_var.set_xlabel('Generation')
    ax_var.set_ylabel(r'$\langle v \rangle$')
    style_axis(ax_var)
    set_equal_aspect(ax_var, ratio_based=True)
    
    # Panel (b): Curvature over time
    ax_curv.plot(time, flat_curv, color=flat_color, linewidth=2)
    ax_curv.plot(time, sharp_curv, color=sharp_color, linewidth=2)
    ax_curv.set_xlabel('Generation')
    ax_curv.set_ylabel('Mean |Hessian Eigenvalue|')
    style_axis(ax_curv)
    set_equal_aspect(ax_curv, ratio_based=True)
    
    # Panel (c): Variance ratio (flat / sharp)
    ratio = flat_var / (sharp_var + 1e-12)
    ax_ratio.plot(time, ratio, color=COLORS[2], linewidth=2)
    ax_ratio.set_xlabel('Generation')
    ax_ratio.set_ylabel('Flat / Sharp Variance')
    style_axis(ax_ratio)
    set_equal_aspect(ax_ratio, ratio_based=True)
    ...
    add_subplot_labels(axes)
    adjust_layout(fig, left=0.1, right=0.95, top=0.85, bottom=0.15, wspace=0.4)
    plt.show()
    
    if save_fig:
        fname = f"{filename_prefix}_flat{flat_k}_sharp{sharp_k}.jpeg"
        save_figure(fig, fname)
    
    return fig


def plot_projected_variance_by_eigenvector(
    alignment_data: Dict,
    snapshot_times: Optional[List[int]] = None,
    save_fig: bool = False,
    filename_prefix: str = "projected_variance_eigenvectors"
) -> plt.Figure:
    """
    Plot variance projected onto each Hessian eigenvector, ordered by eigenvalue.
    
    Shows:
    - (a) Projected variance vs eigenvector index at selected time snapshots
    - (b) Heatmap of projected variance over all eigenvectors and time
    - (c) Corresponding Hessian eigenvalues at each index
    
    Args:
        alignment_data: Output from compute_covariance_hessian_alignment()
        snapshot_times: List of time indices to show in panel (a). Defaults to [0, mid, end].
        save_fig: Whether to save the figure
        filename_prefix: Prefix for saved filename
    ...
    """
    time = alignment_data['time']
    projected_var = alignment_data['projected_variance']  # Shape: (T, D)
    hess_eigs = alignment_data['hessian_eigenvalues']     # Shape: (T, D)
    flat_k = alignment_data['flat_k']
    sharp_k = alignment_data['sharp_k']
    
    T, D = projected_var.shape
    eigenvector_indices = np.arange(D)
    
    # Default snapshot times
    if snapshot_times is None:
        if T >= 3:
            snapshot_times = [0, T // 2, T - 1]
        elif T >= 2:
            snapshot_times = [0, T - 1]
        else:
            snapshot_times = [0]
    
    fig, axes = create_figure(n_cols=3)
    ax_bars, ax_heatmap, ax_eigs = axes
    
    # Panel (a): Projected variance at snapshots
    cmap = plt.cm.viridis
    colors = [cmap(i / (len(snapshot_times) - 1)) if len(snapshot_times) > 1 else cmap(0.5)
              for i in range(len(snapshot_times))]
    
    bar_width = 0.8 / len(snapshot_times)
    for i, t in enumerate(snapshot_times):
        t = min(t, T - 1)
        offset = (i - len(snapshot_times) / 2 + 0.5) * bar_width
        ax_bars.bar(eigenvector_indices + offset, projected_var[t], 
                   width=bar_width, color=colors[i], alpha=0.8,
                   label=f't={int(time[t])}')
    
    # Mark flat vs sharp regions
    ax_bars.axvspan(-0.5, flat_k - 0.5, alpha=0.1, color=COLORS[0], label='Flat')
    ax_bars.axvspan(D - sharp_k - 0.5, D - 0.5, alpha=0.1, color=COLORS[3], label='Sharp')
    
    ax_bars.set_xlabel('Eigenvector Index (ordered by |eigenvalue|)')
    ax_bars.set_ylabel('Projected Variance')
    ax_bars.legend(frameon=False, fontsize=8)
    style_axis(ax_bars)
    
    # Panel (b): Heatmap of variance over time
    im = ax_heatmap.imshow(projected_var.T, aspect='auto', origin='lower',
                           cmap='viridis', 
                           extent=[time[0], time[-1], -0.5, D - 0.5])
    ax_heatmap.set_xlabel('Generation')
    ax_heatmap.set_ylabel('Eigenvector Index')
    
    # Add horizontal lines to mark flat/sharp boundary
    ax_heatmap.axhline(y=flat_k - 0.5, color='white', linestyle='--', linewidth=1, alpha=0.7)
    ax_heatmap.axhline(y=D - sharp_k - 0.5, color='white', linestyle='--', linewidth=1, alpha=0.7)
    
    cbar = fig.colorbar(im, ax=ax_heatmap, shrink=0.8)
    cbar.set_label('Variance', fontsize=10)
    style_axis(ax_heatmap, remove_top_right=False)
    
    # Panel (c): Hessian eigenvalues at final time
    final_eigs = hess_eigs[-1]
    ax_eigs.bar(eigenvector_indices, np.abs(final_eigs), color=COLORS[2], alpha=0.8)
    ax_eigs.axvspan(-0.5, flat_k - 0.5, alpha=0.1, color=COLORS[0])
    ax_eigs.axvspan(D - sharp_k - 0.5, D - 0.5, alpha=0.1, color=COLORS[3])
    ax_eigs.set_xlabel('Eigenvector Index')
    ax_eigs.set_ylabel('|Hessian Eigenvalue|')
    style_axis(ax_eigs)
    
    add_subplot_labels(axes)
    adjust_layout(fig, left=0.08, right=0.95, top=0.9, bottom=0.15, wspace=0.35)
    plt.show()
...
def plot_variance_eigenvalue_anticorrelation(
    alignment_data: Dict,
    snapshot_times: Optional[List[int]] = None,
    save_fig: bool = False,
    filename_prefix: str = "variance_eigenvalue_anticorrelation"
) -> plt.Figure:
    """
    Show anti-correlation between projected variance and |Hessian eigenvalue|.
    
    Panels:
    - (a) log-log scatter of variance vs |eigenvalue| at selected times
    - (b) binned mean trend at final time (log-log)
    - (c) Pearson correlation over time
    
    Args:
        alignment_data: Output from compute_covariance_hessian_alignment()
        snapshot_times: Time indices to show in panel (a). Defaults to [0, mid, end].
        save_fig: Whether to save the figure
        filename_prefix: Prefix for saved filename
    """
    ...
    fig, axes = create_figure(n_cols=3)
    ax_scatter, ax_binned, ax_corr = axes
    
    # Panel (a): log-log scatter at selected times
    cmap = plt.cm.viridis
    colors = [cmap(i / (len(snapshot_times) - 1)) if len(snapshot_times) > 1 else cmap(0.5)
              for i in range(len(snapshot_times))]
    for i, t in enumerate(snapshot_times):
        ...
        mask = (eigs > 0) & (vars_ > 0)
        ax_scatter.scatter(
            eigs[mask], vars_[mask],
            s=12, alpha=0.6, color=colors[i],
            label=f't={int(time[t])}'
        )
    ax_scatter.set_xscale('log')
    ax_scatter.set_yscale('log')
    ax_scatter.set_xlabel(r'$|\lambda_i|$')
    ax_scatter.set_ylabel(r'Projected variance $v_i$')
    ax_scatter.legend(
        frameon=False,
        fontsize=8,
        loc='lower center',
        bbox_to_anchor=(0.5, 1.12),
        ncol=len(snapshot_times)
    )
    style_axis(ax_scatter)
    set_equal_aspect(ax_scatter, ratio_based=True)
    
    # Panel (b): binned trend at final time
    ...
    # Panel (c): Pearson correlation over time (log-log)
    corrs = np.zeros(T)
    for t in range(T):
        eigs = np.abs(hess_eigs[t])
        vars_ = projected_var[t]
    ...
    ax_corr.plot(time, corrs, color=COLORS[3], linewidth=1.8)
    ax_corr.axhline(0, color='black', linewidth=0.8, alpha=0.4)
    ax_corr.set_xlabel('Generation')
    ax_corr.set_ylabel('Pearson r (log-log)')
    style_axis(ax_corr)
    ...
    if save_fig:
        fname = f"{filename_prefix}_D{D}.jpeg"
        save_figure(fig, fname)
